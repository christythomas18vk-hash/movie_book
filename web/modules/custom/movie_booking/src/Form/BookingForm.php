...
class BookingForm extends FormBase {
    ...
    $form['#attached']['html_head'][] = [
      [
        ...
        '#value' => '
          ...
          .seat input[type="checkbox"]:checked + label { background-color: #007bff; }
        ',
      ],
      'seat_map_styles',
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $transaction = \Drupal\node\Entity\Node::create([
      'type' => 'transaction',
      'title' => 'Transaction for Booking #' . $booking_id,
      'field_transaction_amount' => 0, // will update after payment
      'field_booking_reference' => ['target_id' => $booking_id],
      'field_transaction_date' => \Drupal::time()->getRequestTime(),
    ]);

    // Generate a unique Transaction ID (GUID)
    $transaction->set('field_transaction_id', \Drupal::service('uuid')->generate());

    $transaction->save();
    $form_state->setRedirect('movie_booking.payment_page', ['booking_id' => $booking_id]);
  }
